...
export class VRHUDManager {
  ...
  private registerBuiltinDrawers(): void {
    ...
    this.panelDrawers.set('sys-splat',     this.drawSplatPanel.bind(this));
  ...
  /**
   * Splat Graphics panel: full mirror of the desktop SplatGraphicsSection
   * UX, rendered onto the panel canvas. Exposes three control groups:
   *   (1) Spark RAD / autogenerated LODs toggle
   *   (2) LOD Distance / Quality Multiplier (0.5x / 1.0x / 1.5x / 2.0x)
   *   (3) Maximum Splats presets + Reset
   *
   * The 2D canvas at VR scale can't host a focus-based text input, so the
   * custom-number-input that exists on the desktop SplatGraphicsSection
   * is intentionally OMITTED here. Users who need a custom cap use the
   * desktop SettingsModal; VR uses only the 5 canonical preset buckets.
   *
   * Every MaxSplat mutation sets `splatMaxCountUserTouched: true` (via the
   * App.tsx action dispatcher) so the scene-startup auto-apply (200K)
   * doesn't silently re-clamp explicit user choices on the next VR
   * entry/exit cycle.
   *
   * Action IDs:
   *   splat.lodEnabled:toggle
   *   splat.lodScale:<0.5|1.0|1.5|2.0>
   *   splat.maxCount:<number|undefined>
   *   splat.maxCount:reset
   *
   * Layout (1024x768 canvas, body starts at y=180 after chrome):
   *   y=204..216  Subtitle / hint
   *   y=216..296  CARD 1: SPARK RAD toggle (80px)
   *   y=308..394  CARD 2: LOD DISTANCE / QUALITY MULTIPLIER (86px, when LOD on)
   *   y=406..502  CARD 3: MAXIMUM SPLATS (96px)
   *   y=514..544  Bottom hint text
   */
  private drawSplatPanel(
    ctx: CanvasRenderingContext2D,
    w: number,
    _h: number,
    helper: PanelDrawHelper,
    ...
    const bodyTop = helper.drawStandardChrome(
      'SPLAT GRAPHICS',
      'Spark RAD LODs + cap tuning for Gaussian Splat assets',
      '#10b981'
    );
    const g = data.graphicsSettings;
    const splatLodOn = g.splatLodEnabled !== false;
    const currentMax = g.splatMaxCount;
    const currentLodScale = g.splatLodScale ?? 1.0;

    // Subtitle / one-liner description
    ctx.fillStyle = '#94a3b8'; ctx.font = '14px sans-serif';
    ctx.fillText(
      'Live-linked to SparkRenderer.lodSplatCount and Spark LOD scaling. Changes apply next frame.',
      40, bodyTop + 24
    );

    // helper: card backdrop + label
    const drawCard = (top: number, h: number, title: string, accent: string): void => {
      ctx.fillStyle = 'rgba(8,10,18,0.55)';
      ctx.fillRect(40, top, w - 80, h);
      ctx.strokeStyle = accent; ctx.lineWidth = 2;
      ctx.strokeRect(40, top, w - 80, h);
      ctx.fillStyle = accent; ctx.font = 'bold 14px sans-serif';
      ctx.fillText(title.toUpperCase(), 50, top + 22);
    };

    // helper: a single full-width toggle button
    const drawToggle = (x: number, y: number, bw: number, bh: number, on: boolean, label: string, action: string, accent: string): void => {
      ctx.fillStyle = on ? 'rgba(16,185,129,0.28)' : 'rgba(30,41,59,0.7)';
      ctx.fillRect(x, y, bw, bh);
      ctx.strokeStyle = on ? accent : '#475569'; ctx.lineWidth = 2;
      ctx.strokeRect(x, y, bw, bh);
      ctx.fillStyle = on ? '#6ee7b7' : '#cbd5e1';
    ...
    // helper: horizontal row of N preset buttons
    const drawButtonRow = (
      x: number, y: number, cellW: number, cellH: number, gap: number,
      values: Array<string>,
      isActive: (v: string, i: number) => boolean,
      fmt: (v: string, i: number) => string,
      accent: string,
      makeAction: (v: string, i: number) => string
    ): void => {
      values.forEach((v, i) => {
        const cx = x + i * (cellW + gap);
        const on = isActive(v, i);
        ctx.fillStyle = on ? 'rgba(16,185,129,0.28)' : 'rgba(30,41,59,0.7)';
        ...
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillText(fmt(v, i), cx + cellW / 2, y + cellH / 2);
        ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
        helper.registerButton({ x: cx, y, w: cellW, h: cellH }, makeAction(v, i));
      });
    };

    let cursor = bodyTop + 36;

    // === CARD 1: Spark RAD toggle ===
    drawCard(cursor, 80, 'SPARK RAD / AUTOGENERATED LODS', '#10b981');
    drawToggle(
      56, cursor + 36, w - 112, 36,
      splatLodOn,
      `SPARK RAD / LODS: ${splatLodOn ? 'ON' : 'OFF'}`,
      'splat.lodEnabled:toggle',
      '#10b981'
    );
    cursor += 80 + 12;

    // === CARD 2: LOD Distance / Quality Multiplier (only when LOD on) ===
    if (splatLodOn) {
      drawCard(cursor, 86, 'LOD DISTANCE / QUALITY MULTIPLIER', '#10b981');
      drawButtonRow(
        56, cursor + 36, 200, 36, 16,
        ['0.5', '1.0', '1.5', '2.0'],
        (v) => Math.abs(currentLodScale - parseFloat(v)) < 0.01,
        (v) => v + 'x',
        '#10b981',
        (v) => 'splat.lodScale:' + v
      );
    ...
    // === CARD 3: Maximum Splats ===
    const presetCount = 5;
    const presetLabels = ['NO LIMIT', '1,000,000', '500,000', '200K*', '100,000'];
    // Encoded action value: 'undefined' string for No Limit, integer-as-string for others.
    const presetValues: Array<string> = ['undefined', '1000000', '500000', '200000', '100000'];
    drawCard(cursor, 120, 'MAXIMUM SPLATS (LIVE)', '#10b981');
    drawButtonRow(
      56, cursor + 36,
      Math.floor((w - 112) / presetCount) - 4, 36, 8,
      presetValues,
      (raw) => {
        if (raw === 'undefined') return currentMax === undefined;
        return currentMax === parseInt(raw, 10);
      },
      (_, i) => presetLabels[i],
      '#10b981',
      (raw) => 'splat.maxCount:' + raw
    );
    // Reset button (only shown when currentMax !== undefined)
    if (currentMax !== undefined) {
      const resetW = 140;
      const resetH = 28;
      const resetX = w - 56 - resetW;
      const resetY = cursor + 80;
      ...
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText('RESET (NO LIMIT)', resetX + resetW / 2, resetY + resetH / 2);
      ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
      helper.registerButton({ x: resetX, y: resetY, w: resetW, h: resetH }, 'splat.maxCount:reset');
    }
    cursor += 96 + 12;

    // === Bottom hint ===
    ctx.fillStyle = '#10b981'; ctx.font = 'bold 12px sans-serif';
    ctx.fillText('* 200K is the auto-applied VR default on entering immersive mode.', 40, cursor);
    ctx.fillStyle = '#94a3b8'; ctx.font = '12px sans-serif';
    ctx.fillText(
      'Custom Max Splats number: desktop Settings only. Lower on dense scenes & Quest-class headsets.',
      40, cursor + 18
    );
  }
  ...
  public static readonly SYSTEM_CARDS: ReadonlyArray<{ id: string; name: string }> = [
    ...
    { id: 'sys-splat',     name: 'Splat Graphics'    },
